...
sudo apt-get update
sudo apt-get -y install python-virtualenv software-properties-common
sudo apt-get -y install ros-$ROS_NAME-orocos-kdl ros-$ROS_NAME-kdl-parser-py ros-$ROS_NAME-python-orocos-kdl ros-$ROS_NAME-trac-ik ros-$ROS_NAME-cv-bridge

if [ $PYTHON_VERSION == "3" ]; then
    sudo apt-get -y install python-catkin-tools python3-dev python3-catkin-pkg-modules python3-numpy python3-yaml
...
if [ $PYTHON_VERSION == "3" ]; then
	...
	if [ ! -d "$PYROBOT_PYTHON3_WS/src" ]; then
		...
		# Build
		catkin_make --cmake-args -DPYTHON_EXECUTABLE=$(which python) -DPYTHON_INCLUDE_DIR=$(ls usr/include/ | grep python3 | head -n1) -DPYTHON_LIBRARY=$(ls /usr/lib/x86_64-linux-gnu/ | grep libpython3.*\.so\$ | head -n1)
